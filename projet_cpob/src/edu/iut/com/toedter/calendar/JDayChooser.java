 ...
 */
package edu.iut.com.toedter.calendar;
	...
	public JDayChooser(boolean weekOfYearVisible) {
		...
		for (int y = 0; y < 7; y++) {
			for (int x = 0; x < 7; x++) {
				...
				//[MODIFICATION EFFECTUEE] Si on clique sur un jours on bascule sur l'onglet vueJour correspondant
				for(int i = 0; i < index; i ++){	
					days[i].addActionListener(new ActionListener(){
						@Override
						public void actionPerformed(ActionEvent e) {
							vueAgenda.pressButton(vueAgenda.getBtnJours());
						}
